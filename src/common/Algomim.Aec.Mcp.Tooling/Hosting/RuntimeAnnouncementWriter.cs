...
/// <summary>Writes and removes live MCP host announcement files for runtime discovery.</summary>
public sealed class RuntimeAnnouncementWriter
{
    ...
    public RuntimeAnnouncementWriter(IMcpLogger? logger = null)
    {
    ...
    public void Remove(string owner, int pid)
    {
        try
        {
            ...
            foreach (var file in System.IO.Directory.GetFiles(Directory_, $"{owner}-{pid}-*.json"))
                DeleteIfExists(file);

            _logger.Info($"MCP announcement removed for {owner} pid {pid}");
        }
        catch (Exception ex)
        {
    ...
    {
        entry.Owner = string.IsNullOrWhiteSpace(entry.Owner) ? "aec" : entry.Owner.Trim();
        entry.Id = string.IsNullOrWhiteSpace(entry.Id) ? $"{entry.Owner}-{entry.Pid}-{entry.Port}" : entry.Id.Trim();
        entry.Name = string.IsNullOrWhiteSpace(entry.Name) ? $"{entry.Owner} {entry.Pid}" : entry.Name.Trim();
    ...
    private static void DeleteIfExists(string? path)
    {
        ...
        {
            // Announcement cleanup is best-effort.
        }
    }
}
